...
[Play online at N-Back Studio](https://www.nbackstudio.com/) · [How to play](https://www.nbackstudio.com/how-to-play/)
...
The public site is [nbackstudio.com](https://www.nbackstudio.com/). Local changes are not automatically published; deploy only when explicitly requested.
...
The homepage keeps the game in the first viewport and includes a short, static introduction below it. The standalone `/how-to-play/` guide contains examples, controls, scoring, and common questions. Both pages ship readable HTML and CSS without JavaScript; JavaScript is required only to play. The existing quick-help dialog remains available inside the game.

Page titles, descriptions, and sharing metadata are generated by `build/seo.ts`. Production URLs default to `https://www.nbackstudio.com/`:

1. Run `npm run build`. This generates absolute canonical URLs for both pages, `dist/sitemap.xml`, and `dist/robots.txt` with a sitemap reference. No additional configuration is needed for nbackstudio.com.
2. If the public address changes, copy `.env.example` to `.env.local` and set `SITE_URL` to the new HTTPS address before building. Include any hosting subdirectory and a trailing slash. Empty, non-HTTPS, and localhost overrides are rejected.
3. Serve `dist/` over HTTPS. Both `/` and `/how-to-play/` must serve their own `index.html` with HTTP 200. Redirect `/how-to-play` to `/how-to-play/` to keep relative links correct, and return HTTP 404 for unknown paths instead of rewriting every URL to the game.
4. If hosting in a subdirectory, merge the generated sitemap reference into the **origin's** `/robots.txt`; crawlers only use the robots file at the origin root. Ensure host-level authentication, `noindex` headers, or crawler restrictions are disabled on the public site. Redirect alternate domains to the preferred domain.
5. The README links to the live site and guide. The GitHub repository's Website field can also point to `https://www.nbackstudio.com/`.

Neither Google accounts nor analytics are needed for these changes. The sitemap helps discovery but does not guarantee indexing or rankings. Never publish a build configured with a preview or placeholder address as its canonical URL.
